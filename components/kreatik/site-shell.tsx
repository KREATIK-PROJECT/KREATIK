    ...
    <header className="relative z-50 border-b border-white/10 bg-[#12141C]/95 px-5 py-5 backdrop-blur-md md:px-10">
      <div className="mx-auto flex max-w-[1440px] items-center justify-between">
        ...
        <nav className="hidden items-center gap-8 font-montserrat text-[10px] font-bold tracking-[0.16em] text-white/70 md:flex" aria-label="Navigation principale">
          {['BOUTIQUE', 'ARTISTES', 'COLLECTIONS', 'ATELIER', 'DROPS', 'FONDATEUR'].map((item) => <a className="transition-colors hover:text-[#F77F4A]" href={`#${item.toLowerCase().replace(' ', '-')}`} key={item}>{item}</a>)}
        </nav>
      ...
      {menuOpen && <nav className="mt-5 flex flex-col gap-5 border-t border-white/10 pt-5 font-montserrat text-xs font-bold tracking-[0.18em] text-white/80 md:hidden">{['BOUTIQUE', 'ARTISTES', 'COLLECTIONS', 'ATELIER', 'DROPS', 'FONDATEUR'].map((item) => <a href={`#${item.toLowerCase().replace(' ', '-')}`} onClick={() => setMenuOpen(false)} key={item}>{item}</a>)}</nav>}
    </header>
  )
}
